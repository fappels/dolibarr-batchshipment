...
/**
 *  \file       mastershipment_document.php
 *  \ingroup    batchshipment
 *  \brief      Tab for documents linked to MasterShipment
 */
...
$langs->loadLangs(array("batchshipment@batchshipment", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->batchshipment->multidir_output[$object->entity ? $object->entity : $conf->entity]."/mastershipment/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('batchshipment', 'mastershipment', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("MasterShipment")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-batchshipment page-card_document');

// Show tabs
$head = mastershipmentPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("MasterShipment"), -1, $object->picto);
...
$modulepart = 'batchshipment';
$param = '&id='.$object->id;
//$relativepathwithnofile='mastershipment/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'mastershipment/'.dol_sanitizeFileName($object->ref).'/';
