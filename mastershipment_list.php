...
/**
 *   	\file       mastershipment_list.php
 *		\ingroup    batchshipment
 *		\brief      List page for mastershipment
 */
...
require_once __DIR__.'/class/mastershipment.class.php';
// for other modules
...
$langs->loadLangs(array("batchshipment@batchshipment", "other"));
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('batchshipment', 'mastershipment', 'write');
	$permissiontodelete = $user->hasRight('batchshipment', 'mastershipment', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("batchshipment")) {
	accessforbidden('Module batchshipment not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'MasterShipment';
	$objectlabel = 'MasterShipment';
	$uploaddir = $conf->batchshipment->dir_output;
...
$title = $langs->trans("MasterShipments");
//$help_url = "EN:Module_MasterShipment|FR:Module_MasterShipment_FR|ES:Módulo_MasterShipment";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/batchshipment/mastershipment_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-batchshipment page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/batchshipment/mastershipment_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendMasterShipmentRef";
$modelmail = "mastershipment";
$objecttmp = new MasterShipment($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if MASTERSHIPMENT_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
